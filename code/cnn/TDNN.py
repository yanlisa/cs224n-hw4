...
class TDNN(Model):
  ...
  def __init__(self, input_, embed_dim=650,
    ...
    for idx, kernel_dim in enumerate(kernels):
      ...
      #print("after pooling for {}, {}:{}".format(idx, kernel_dim, pool.get_shape()))

      layers.append(tf.squeeze(pool, [1,2])) # [?,kernel_dim]

    self.output = tf.concat(1, layers) # [?,sum(kernel_dims)]
    #print("TDNN output", self.output.get_shape())
